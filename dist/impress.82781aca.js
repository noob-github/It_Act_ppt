...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"impress.js":[function(require,module,exports) {
...
(function (document, window) {
  ...
  var impress = window.impress = function (rootId) {
    ...
    var initialized = false; // STEP EVENTS
    //
    ...
    var stepEnterTimeout = null; // `goto` API function that moves to step given as `el` parameter (by index, id or element).
    ...
    var swipe = function swipe(pct) {
      ...
      event.detail.swipe = pct; // Will be ignored within swipe animation, but just in case a plugin wants to read this,
      // humor them
  ...
  impress.supported = impressSupported; // ADD and INIT LIBRARIES
...
})(document, window); // THAT'S ALL FOLKS!
//
...
(function (document, window) {
  ...
  var recordStartingState = function recordStartingState(rootId) {
    ...
    if (rootsCount === 0) {
      startingState.body = {}; // It is customary for authors to set body.class="impress-not-supported" as a starting
  ...
  var resetStartingState = function resetStartingState(rootId) {
    ...
    document.body.style.overflow = ""; // Remove style values from the root and step elements
    ...
    delete startingState.roots[rootId]; // Move step div elements away from canvas, then delete canvas
...
(function (document) {
  ...
  document.addEventListener("impress:init", function (event) {
    util = event.detail.api.lib.util; // Getting API from event data instead of global impress().init().
    // You don't even need to know what is the id of the root element
    // or anything. `impress:init` event data gives you everything you
    // need to control the presentation that was just initialized.

    api = event.detail.api;
    root = event.target; // Element attributes starting with "data-", become available under
  ...
  document.addEventListener("impress:autoplay:play", function (event) {
    status = "playing";
    reloadTimeout(event);
  }, false); // If default autoplay time was defined in the presentation root, or
  ...
  var preInit = function preInit() {
    if (window.markdown) {
      ...
      for (var idx = 0; idx < markdownDivs.length; idx++) {
        ...
        element.innerHTML = markdown.toHTML(slides[i], dialect); // If there's an id, unset it for last, and all other, elements,
...
(function (document, window) {
  ...
  var goto = function goto(event) {
    ...
    if (isNumber(data.gotoNext) && event.detail.reason === "next") {
      event.detail.next = steps[data.gotoNext]; // If the new next element has its own transitionDuration, we're responsible for setting
...
(function (document) {
  ...
  document.addEventListener("impress:init", function (event) {
    ...
    var api = event.detail.api;
    var gc = api.lib.gc;
    var util = api.lib.util; // Supported keys are:
...
(function (document, window) {
  ...
  var sortSubsteps = function sortSubsteps(substepNodeList) {
    var substeps = Array.from(substepNodeList);
    var sorted = substeps.filter(function (el) {
      return el.dataset.substepOrder;
    }).sort(function (a, b) {
      var orderA = a.dataset.substepOrder;
      var orderB = b.dataset.substepOrder;
      return parseInt(orderA) - parseInt(orderB);
    }).concat(substeps.filter(function (el) {
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "40331" + '/');
...
},{}]},{},["node_modules/parcel-bundler/src/builtins/hmr-runtime.js","impress.js"], null)
//# sourceMappingURL=/impress.82781aca.js.map